...
export default function StudentActivity() {
    ...
    const loadData = useCallback(async () => {
        let response = await fetch(`/api/guide-teams`);
        if (!response.ok)
            throw { status: response.status, msg: response.statusText };
        let guideTeams = await response.json();
        ...
        addStudentInfoToGuideTeams(student, guideTeams);
        setStudent(student);
    }, [id, fetch]);
    ...
    function setGuideTeamLoading(cell) {
        let newGuideTeams = [...guideTeams];
        newGuideTeams[
            newGuideTeams.findIndex((s) => s._id == cell.row.original._id)
        ].loading = true;
        setGuideTeams(newGuideTeams);
    }

    function removeGuideTeamLoading(cell) {
        let newGuideTeams = [...guideTeams];
        newGuideTeams[
            newGuideTeams.findIndex((s) => s._id == cell.row.original._id)
        ].loading = false;
    ...
    async function handleAssign(event, cell) {
        if (event.target.checked) {
            setGuideTeamLoading(cell);
            let response = await fetch(
                `/api/students/${student._id}/guideTeam/${cell.row.original._id}`,
                {
                    method: "DELETE",
                    headers: {
                        "Content-Type": `application/json`,
            ...
            if (response.ok) {
                showToast(
                    "Success",
                    "",
                    "Successfully removed Student from Guide Team!",
                    "Success"
                );
                loadData();
            } else {
            ...
            if (response.ok) {
                showToast(
                    "Success",
                    "",
                    "Successfully added Student to Guide Team!",
                    "Success"
                );
                loadData();
            } else {
    ...
    async function handleAssignLeader(event, cell) {
        if (event.target.checked) {
            ...
            setGuideTeamLoading(cell);
            let response = await fetch(
                `/api/students/${student._id}/guideLeader/${cell.row.original._id}`,
                {
                    method: "PUT",
                    headers: {
                        "Content-Type": `application/json`,
            ...
            if (response.ok) {
                showToast(
                    "Success",
                    "",
                    "Successfully assigned student as Guide Team leader!",
                    "Success"
                );
                loadData();
            } else {
    ...
    function addStudentInfoToGuideTeams(student, guideTeams) {
        for (let i in guideTeams) {
            if (
                student.guideTeams.some(
                    (guideTeam) => guideTeam.teamId == guideTeams[i]._id
                )
            ) {
                guideTeams[i].includesStudent = true;
                guideTeams[i].includesStudentAsLeader = student.guideTeams.some(
                    (guideTeam) =>
                        guideTeam.teamId == guideTeams[i]._id &&
                        guideTeam.isLeader == true
                );
            } else {
                guideTeams[i].includesStudent = false;
                guideTeams[i].includesStudentAsLeader = false;
            }
        }
        setGuideTeams(guideTeams);
    }
