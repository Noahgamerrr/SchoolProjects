...
export default function StationRowCompressed({ station, updateSelectCallback }) {
    ...
    function findLastTimeInstance(data) {
        ...
        const lastTimeEntry = data.reduce((latest, current) => {
            const latestTime = new Date(`1970-01-01T${latest.name}:00`);
            const currentTime = new Date(`1970-01-01T${current.name}:00`);
    ...
    let occupancy = findLastTimeInstance(station.visitorsAtTime);
    ...
    if (station.capacity > 10) {
        ...
        for (let x = 0; x < station.capacity; x++) {
            if (x < occupancy) {
                capacityList.push(<i className="bi bi-person-standing text-secondary"></i>);
            } else {
            ...
            <div className="rounded mb-2 p-3">
                <div className="gap-1 d-flex">
                    <Button
                        ...
                        onClick={() => updateSelectCallback(station, "stationTeam")}
                    >
                        <i className="bi bi-search fs-5"></i>
                    </Button>
                    <h4 style={{marginTop: "6px"}}>{station.name}</h4>
                </div>
